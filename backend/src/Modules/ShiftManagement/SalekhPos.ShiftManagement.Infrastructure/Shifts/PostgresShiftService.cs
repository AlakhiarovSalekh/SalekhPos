...
public sealed class PostgresShiftService(NpgsqlDataSource? source) : IShiftService
{
    ...
    {
        Validate(identity, command.OrganizationId, command.BranchId, command.OperationId);
        if (command.ShiftId == Guid.Empty || command.MovementId == Guid.Empty || command.RegisterId == Guid.Empty || command.Kind is not ("cash_in" or "cash_out") || command.Amount <= 0 || decimal.Round(command.Amount, 6) != command.Amount || string.IsNullOrWhiteSpace(command.Reason) || command.Reason.Length is < 3 or > 500) throw new ArgumentException("Cash movement request is invalid.");
        ...
        var shift = await ShiftState(connection, transaction, command.OrganizationId, command.BranchId, command.ShiftId, cancellationToken) ?? throw new ShiftConflictException();
        if (shift.RegisterId != command.RegisterId) throw new ShiftConflictException();
        ...
        if (shift.Status != "open") throw new ShiftConflictException();
    ...
    {
        Validate(identity, command.OrganizationId, command.BranchId, command.OperationId);
        if (command.ShiftId == Guid.Empty || command.RegisterId == Guid.Empty || command.CountedCash < 0 || decimal.Round(command.CountedCash, 6) != command.CountedCash) throw new ArgumentException("Shift closing request is invalid.");
        var dataSource = source ?? throw new ShiftUnavailableException(); await using var connection = await dataSource.OpenConnectionAsync(cancellationToken); await using var transaction = await connection.BeginTransactionAsync(cancellationToken);
        ...
        var expected = shift.OpeningBalance + sales - refunds + cashIn - cashOut; var variance = command.CountedCash - expected; var closedAt = await Time(connection, transaction, cancellationToken);
        ...
        var response = new ClosedShiftResponse(command.ShiftId, command.BranchId, shift.RegisterId, "closed", shift.Currency, shift.OpeningBalance, sales, refunds, cashIn, cashOut, expected, command.CountedCash, variance, shift.OpenedAt, closedAt, shift.OpenedBy, identity.Subject);
    ...
    private static async Task<ShiftStateRow?> ShiftState(NpgsqlConnection connection, NpgsqlTransaction transaction,
        Guid organizationId, Guid branchId, Guid shiftId, CancellationToken cancellationToken)
    {
        await using var query = new NpgsqlCommand("SELECT register_id,status,currency,opening_balance,opened_at,opened_by FROM shifts.shifts WHERE organization_id=$1 AND branch_id=$2 AND shift_id=$3", connection, transaction);
        query.Parameters.AddWithValue(organizationId); query.Parameters.AddWithValue(branchId); query.Parameters.AddWithValue(shiftId);
        await using var reader = await query.ExecuteReaderAsync(cancellationToken);
        return await reader.ReadAsync(cancellationToken)
            ? new(reader.GetGuid(0), reader.GetString(1), reader.GetString(2), reader.GetDecimal(3),
                reader.GetFieldValue<DateTimeOffset>(4), reader.GetString(5))
            : null;
    }

    private sealed record ShiftStateRow(Guid RegisterId, string Status, string Currency, decimal OpeningBalance,
        DateTimeOffset OpenedAt, string OpenedBy);
