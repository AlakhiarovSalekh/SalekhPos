...
export function SessionPanel({ dashboard = false }: { dashboard?: boolean }) {
  ...
  if (session.authenticated) return <><p className="eyebrow">SIGNED IN</p><h1>{dashboard ? "Welcome back" : "You’re signed in"}<span className="accent">.</span></h1><p className="description">{session.name || "Your account"}</p>{dashboard ? <><div className="dashboard-actions"><Link className="button" href="/sales">Manage sales →</Link><Link className="secondary-button" href="/returns">Return history</Link></div><div className="notice"><h2>Inventory management</h2><p>Review current stock and record authorized receipts or adjustments through your secure web session.</p><Link className="button compact" href="/inventory">Open inventory →</Link></div><div className="notice"><h2>Product catalog</h2><p>Manage the products your stores use at checkout.</p><Link className="button compact" href="/products">Manage products →</Link></div></> : <Link className="button" href="/dashboard">Open workspace →</Link>}<form action="/auth/logout" method="post"><input type="hidden" name="__RequestVerificationToken" value={session.csrfToken ?? ""} /><button className="text-button" type="submit">Sign out</button></form></>;
