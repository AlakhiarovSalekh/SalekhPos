...
export function SessionPanel({ dashboard = false }: { dashboard?: boolean }) {
  ...
  if (session.authenticated) return <><p className="eyebrow">SIGNED IN</p><h1>{dashboard ? "Welcome back" : "You’re signed in"}<span className="accent">.</span></h1><p className="description">{session.name || "Your account"}</p><div className="notice"><h2>Inventory management</h2><p>Review current stock and record authorized receipts or adjustments through your secure web session.</p><Link className="button compact" href="/inventory">Open inventory →</Link></div>{!dashboard && <Link className="button" href="/dashboard">Open workspace →</Link>}<form action="/auth/logout" method="post"><input type="hidden" name="__RequestVerificationToken" value={session.csrfToken ?? ""} /><button className="text-button" type="submit">Sign out</button></form></>;
