...
The desktop now has an explicit readiness stage between trusted-device authentication/provisioning and cashier mode. Online readiness first reconciles pending local sales, refreshes authoritative cash-session state, and always refreshes the sellable catalog even when no cash session is open. Local sellable-catalog readiness is persisted per organization and branch using canonical UTC snapshot metadata and SHA-256 digest state; missing or corrupt readiness fails closed, while a valid authoritative empty catalog is considered ready. POS sale eligibility now requires both an active cash session and a verified local catalog projection.

Transient network/server failures may enter offline cashier mode only when previously verified local catalog data exists for the exact organization/branch. A brand-new terminal without a trusted projection remains in a bounded retry/sign-out readiness UI and cannot reach cashier mode. Reauthentication remains distinct from offline fallback, readiness attempts cannot publish the runtime twice, cancellation/close releases owned resources, and the cashier consumes the already-prepared workspace state instead of performing a second startup download. Manual synchronization now reconciles pending sales before refreshing both authoritative session and catalog state.

The complete Native PostgreSQL CI gate passes all 464 requirement ranges, all 1,543 required structure entries, architecture coverage for 71 projects, working-tree and 83-commit Gitleaks scans, locked restore, Release build with zero warnings/errors, formatting, dependency audit with no known vulnerable direct/transitive NuGet dependencies, fresh PostgreSQL migration/RLS/isolation checks, logical backup/restore, 167 core tests, 105 desktop tests and 73 integration tests, with zero failures or skips (345 total tests). Exact next slice: close the remaining cashier operational gap around establishing an active cash session on a trusted register so a freshly prepared terminal can move from 'catalog ready, no open cash session' into an authorized sellable state without manual database/bootstrap intervention, while preserving idempotency, register/device binding, authorization, audit and offline safety.
## September 13, 2026 - interactive trusted desktop runtime provisioning

The desktop runtime no longer trusts static branch or device identifiers from environment configuration. After native OIDC authentication it first attempts to resume the locally persisted trusted terminal assignment, verifies that the assignment belongs to the configured organization, opens the referenced OS-protected key without creating a replacement, validates canonical NIST P-256 public-key material and checks its SHA-256 fingerprint in constant time. Missing, corrupt, mismatched or cross-deployment local trust state fails closed before a POS workspace is published.

First-time setup now discovers all permitted branches and active registers through authenticated paged APIs, rejects duplicate/cyclic pagination, malformed or oversized responses, cross-branch register data and unexpected JSON, and preserves the separate organization/business/optional-region hierarchy returned by the backend. The Avalonia setup flow lets the authenticated operator choose an authorized branch and active register, derives the desktop platform locally, runs the existing secure register-to-trust provisioning protocol, and constructs the POS workspace only after an active trusted device is returned. Failed or cancelled setup does not publish a workspace and remains retryable; sign-out and window closure release credentials and authenticated resources.

The complete Native PostgreSQL CI gate passes all 464 requirement ranges, all 1,543 required structure entries, architecture coverage for 71 projects, working-tree and 81-commit Gitleaks scans, locked restore, Release build with zero warnings/errors, formatting, dependency audit with no known vulnerable direct/transitive NuGet dependencies, fresh PostgreSQL migration/RLS/isolation checks, logical backup/restore, 167 core tests, 92 desktop tests and 73 integration tests, with zero failures or skips (332 total tests). Exact next slice: make first-run POS readiness explicit after provisioning/resume by performing a resumable initial synchronization, validating the resulting local catalog/price/stock state, surfacing bounded progress/retry/failure states, and entering cashier mode only when the local offline projection is safe to use.
## September 13, 2026 - device-bound sync and secure desktop provisioning

Synchronization is now cryptographically bound to an active trusted terminal credential on every device-sensitive sync request. The request proof is domain-separated and binds the authenticated human identity context, organization, branch, device, credential, HTTP method, canonical path, operation identity, exact body digest, timestamp and fresh nonce. The server verifies ECDSA P-256 IEEE-P1363 signatures against the active device credential, enforces bounded clock skew and durable PostgreSQL nonce replay protection, and rejects missing, malformed, pending, revoked, cross-device, modified or replayed proofs. Shared terminals are no longer tied to the human operator who originally enrolled them: independent bearer authorization still governs what the current cashier may do, while the terminal identity is proven by its device key.

The Windows desktop now provisions a persistent non-exportable ECDSA P-256 device key through CNG, persists only non-secret device binding metadata, completes the pending-to-trusted enrollment proof, verifies persisted key/fingerprint consistency on reuse, and signs sync requests with the OS-protected key. Exact serialized request bytes are hashed before signing; retries keep the business message stable while issuing a fresh timestamp, nonce and signature. Missing/corrupt provisioning state, a missing OS key, or fingerprint mismatch fail closed before a network request is sent. Cross-module proof verification is composed in the Bootstrapper host so Sync remains independent of Devices implementation details.

The complete Native PostgreSQL CI gate passes all 464 requirement ranges, all 1,543 required structure entries, architecture coverage for 71 projects, working-tree and 79-commit Gitleaks scans, locked restore, Release build with zero warnings/errors, formatting and dependency vulnerability checks, fresh PostgreSQL migration/RLS/isolation checks, logical backup/restore, 167 core tests, 80 desktop tests and 73 integration tests, with zero failures or skips (320 total tests). The verified milestone includes commits `3b59c39`, `9d0f4e0`, `82e94f4`, `0fc0691` and `e26b81f` plus final analyzer/format cleanup. Exact next slice: wire the secure provisioning service into the real desktop runtime flow so an authenticated operator can discover permitted branches, select an active register, provision or resume the terminal identity, then enter initial sync without static device assignment.

## September 12, 2026 - cryptographic device trust lifecycle and revocation

Device enrollment now separates registration from trust. Authorized `devices.manage` operators register a terminal idempotently with a canonical ECDSA P-256 SubjectPublicKeyInfo public key; the server creates the device as `pending`, generates a cryptographically random 32-byte proof challenge with a bounded lifetime, and stores no private-key material. Canonical server-side SHA-256 fingerprints are persisted as exactly 32 bytes and are unique among pending/active credentials inside an organization, preventing a live credential from silently enrolling multiple terminals.

Trust establishment is fail-closed and provider-neutral. The server explicitly requires the `secp256r1` / NIST P-256 curve OID and 32-byte affine coordinates, accepts only exact 64-byte IEEE-P1363 ECDSA signatures, and verifies a domain-separated proof binding organization, branch, device, credential, trust operation, the server-stored challenge and the server-computed fingerprint. Registration replay compares canonical public-key material in constant time; malformed keys/proofs, wrong curves, changed idempotent material, duplicate live keys and mismatched operation/fingerprint proofs are rejected without activating the device. Public-key bytes and fingerprints are not exposed in normal device responses.

Explicit device revocation atomically records who revoked the device, when, why and under which idempotent operation, revokes its live credential and prevents later trust re-establishment through the activation path. Pending and revoked terminals cannot synchronize; active trusted terminals can, while pre-existing legacy active rows without credential records remain temporarily compatible for migration safety. Credential tables enforce tenant RLS/FORCE RLS and least-privilege runtime grants, including immutable public key, fingerprint and challenge material after insertion.

The complete Native PostgreSQL CI gate passes all 464 requirement ranges, all 1,543 required structure entries, architecture coverage for 71 projects, working-tree and Git-history Gitleaks scans, locked restore, Release build with zero warnings/errors, dependency vulnerability checks, formatting, fresh PostgreSQL migration/RLS/isolation checks, logical backup/restore, 167 core unit/configuration/HTTP tests, 56 desktop tests and 72 integration tests, with zero failures or skips (295 total tests). Current limitation: trust enrollment proves possession during activation, but normal device-sensitive requests are not yet cryptographically bound to the device key, and synchronization still binds the current user to the original device registrant; that is not suitable as the final model for shared cashier terminals. Exact next slice: add per-request device proof-of-possession with replay protection and decouple trusted terminal identity from the human operator while preserving independent user authorization/audit, then wire the desktop client to an OS-protected device key and controlled provisioning flow.
